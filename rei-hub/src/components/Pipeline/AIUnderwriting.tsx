  ...
  return (
    <div className={`relative w-36 h-36 ${bg} rounded-full flex items-center justify-center`}>
      ...
      <div className="text-center z-10">
        ...
        <p className="text-xs text-slate-500 font-medium">/ 100</p>
      </div>
    </div>
  )
}
    ...
    <div className={`rounded-xl border ${bannerBg} overflow-hidden`}>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-slate-600 mt-0.5 truncate">{bannerSubtext}</p>
        </div>
        <span className="text-sm text-slate-400">{expanded ? 'Hide' : 'Details'}</span>
      </button>
        ...
        <div className="border-t border-slate-200 p-4 space-y-3">
            ...
            <div className="space-y-2">
              {validation.checks.map((check, i) => (
                <div key={i} className="flex items-start gap-2 p-2 bg-white rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-800">{check.metric}</p>
                    <p className="text-sm text-slate-500 mt-0.5">{check.note}</p>
          ...
          {validation.discrepancies.length > 0 && (
            <div className="bg-red-50 rounded-lg p-3">
              <p className="text-sm font-semibold text-red-700 mb-1">Discrepancies:</p>
...
function AiDisclaimer() {
  return (
    <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start gap-2.5">
      ...
      <div>
        <p className="text-sm font-semibold text-amber-800">AI-Generated Analysis — Not Financial Advice</p>
    ...
    return (
      <div className="space-y-4">
        <div className="bg-gradient-to-br from-purple-50 to-blue-50 rounded-xl border border-purple-200 p-8 text-center">
          ...
          <p className="text-base text-slate-600 mb-1 max-w-md mx-auto">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={runAnalysis}
            disabled={running}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-purple-700 bg-purple-50 rounded-lg hover:bg-purple-100 disabled:opacity-50 transition-colors"
          >
            ...
            <div className="flex flex-wrap gap-3 mt-2">
              {analysis.recommended_offer != null && (
                <div className="bg-blue-50 rounded-lg px-3 py-2">
                  <p className="text-xs text-blue-600 font-medium uppercase">Recommended Offer</p>
              ...
              {analysis.max_allowable_offer != null && (
                <div className="bg-slate-50 rounded-lg px-3 py-2">
                  <p className="text-xs text-slate-500 font-medium uppercase">Max Allowable</p>
      ...
      {analysis.comp_analysis && analysis.comp_analysis.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h4 className="text-base font-semibold text-slate-800 mb-3">Comparable Sales</h4>
              ...
              <tbody>
                {analysis.comp_analysis.map((comp, i) => (
                  <tr key={i} className="border-b border-slate-100">
                    ...
                    <td className="px-3 py-2 text-slate-600 text-sm">{comp.relevance}</td>
                  </tr>
                ))}
              </tbody>
            </table>
