...
export function generateHTML(config: TemplateConfig): string {
  const color = config.primary_color || '#4d7c0f'
  const colorDark = adjustBrightness(color, -20)
  const colorLight = adjustBrightness(color, 20)
  const heroImage = heroImages.land
  const avatars = avatarImages.land
...
<body>
  ...
  <section class="hero">
    <div class="hero-container">
      <div class="hero-content">
        <div class="hero-badge">${renderIcon('mapPin', 16)} Land Buying Specialists</div>
        <h1>${config.headline}</h1>
        <p>${config.description}</p>
        <div class="hero-stats">
          <div class="hero-stat"><span class="number">500+</span><span class="label">Acres Purchased</span></div>
          <div class="hero-stat"><span class="number">All Property</span><span class="label">Types</span></div>
          <div class="hero-stat"><span class="number">Fair Cash</span><span class="label">Offers</span></div>
        </div>
      </div>
      <div class="form-card">
        <h2 class="form-title">Get Your Land Offer</h2>
        <p class="form-subtitle">Any size, any location. Fair cash offers on vacant land..</p>
        <form id="leadForm">
  ...
  <section class="section how-it-works" id="how-it-works">
    <div class="section-container">
      <div class="section-header">
        ...
        <p class="section-subtitle">Selling your land has never been easier. Just three simple steps.</p>
      </div>
      <div class="steps-grid">
        <div class="step">
          <span class="step-number">01</span>
          <div class="step-icon">${renderIcon('clipboard', 36)}</div>
          <h3>Submit Land Details</h3>
          <p>Tell us about your land. Size, location, zoning, condition. We evaluate everything.</p>
        </div>
        <div class="step">
          <span class="step-number">02</span>
          <div class="step-icon">${renderIcon('dollar', 36)}</div>
          <h3>Get Cash Offer</h3>
  ...
  <section class="section benefits" id="benefits">
    <div class="section-container">
      <div class="section-header">
        ...
        <p class="section-subtitle">We buy all types of land at fair prices.</p>
      </div>
      <div class="benefits-grid">
        <div class="benefit-card">
          <div class="benefit-icon">${renderIcon('target', 26)}</div>
          <div><h3>Any Size Property</h3><p>From small lots to hundreds of acres—we buy it all.</p></div>
        </div>
        <div class="benefit-card">
          <div class="benefit-icon">${renderIcon('home', 26)}</div>
          <div><h3>Raw or Developed</h3><p>Zoned or unzoned, raw or developed land.</p></div>
        </div>
        <div class="benefit-card">
          <div class="benefit-icon">${renderIcon('dollar', 26)}</div>
          <div><h3>No Listing Fees</h3><p>No agent commissions. No hidden fees. Direct sale to us.</p></div>
        </div>
        <div class="benefit-card">
          <div class="benefit-icon">${renderIcon('clock', 26)}</div>
          <div><h3>Quick Closing</h3><p>Close fast without the typical real estate delays.</p></div>
        </div>
      </div>
    </div>
  </section>
    ...
    <div class="section-container">
      ...
      <div class="testimonials-grid">
        <div class="testimonial-card">
          ...
          <p class="testimonial-text">"I had vacant land that wasn't selling. They gave me a fair offer and closed quickly. Couldn't ask for better service!"</p>
          <div class="testimonial-author">
            <img src="${avatars[0]}" alt="William Scott" class="testimonial-avatar">
            <div><div class="testimonial-name">William Scott</div><div class="testimonial-title">Land Owner${config.market ? `, ${config.market}` : ''}</div></div>
          </div>
        </div>
        <div class="testimonial-card">
          <div class="testimonial-stars">${renderStars(5)}</div>
          <p class="testimonial-text">"My family inherited farmland and didn't know what to do. These folks made it easy to sell. Very professional team."</p>
          <div class="testimonial-author">
            <img src="${avatars[1]}" alt="Margaret Lewis" class="testimonial-avatar">
            <div><div class="testimonial-name">Margaret Lewis</div><div class="testimonial-title">Inherited Property Seller${config.market ? `, ${config.market}` : ''}</div></div>
          </div>
        </div>
        <div class="testimonial-card">
          <div class="testimonial-stars">${renderStars(5)}</div>
          <p class="testimonial-text">"Fast closing, fair price, no hidden fees. If you have land to sell, this is definitely the place. Highly recommend!"</p>
          <div class="testimonial-author">
            <img src="${avatars[2]}" alt="David Taylor" class="testimonial-avatar">
            <div><div class="testimonial-name">David Taylor</div><div class="testimonial-title">Land Seller${config.market ? `, ${config.market}` : ''}</div></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="cta-banner">
    <h2>Ready to Sell Your Land?</h2>
    <p>Join our land sellers who got fair offers and quick closings.</p>
    <a href="#" class="cta-btn" onclick="document.getElementById('leadForm').scrollIntoView({behavior:'smooth'});return false;">Get a Land Offer →</a>
  </section>
  ...
  <footer>
    <div class="footer-container">
      <div class="footer-brand">
        ...
        <p>We buy land fast for cash. Vacant lots, acreage, and more${config.market ? `. Proudly serving ${config.market} and surrounding areas` : ''}.</p>
  ...
  <script>
    ...
    document.getElementById('leadForm').addEventListener('submit', function(e) {
      ...
      .finally(function() {
        btn.disabled = false;
        btn.textContent = 'Get a Land Offer \\u2192';
      });
    });
