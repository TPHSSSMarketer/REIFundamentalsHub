...
async def process_pending_tracking(db_session_factory, settings) -> None:
    ...
    1. NegotiationCorrespondence: USPS and Fax tracking (legacy - may be deleted)
    ...
    async with db_session_factory() as db:
        try:
            ...
            # ── Process NegotiationCorrespondence records (legacy) ──
            try:
                ...
                logger.warning(
                    "NegotiationCorrespondence model not found — skipping legacy correspondence tracking"
                )
