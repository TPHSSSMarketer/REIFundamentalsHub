...
def _contact_to_dict(c: CrmContact) -> dict:
    return {
        ...
        "phone": c.phone,
        "email": c.email,
        "tags": c.tags_json if isinstance(c.tags_json, (list, dict)) else (json.loads(c.tags_json) if c.tags_json else []),
        "markets": c.markets_json if isinstance(c.markets_json, (list, dict)) else (json.loads(c.markets_json) if c.markets_json else []),
        "source": c.source,
