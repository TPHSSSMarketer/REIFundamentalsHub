...
def _criteria_to_dict(bc: BuyerCriteria) -> dict:
    return {
        ...
        "propertyTypes": bc.property_types_json if isinstance(bc.property_types_json, (list, dict)) else (json.loads(bc.property_types_json) if bc.property_types_json else []),
        "markets": bc.markets_json if isinstance(bc.markets_json, (list, dict)) else (json.loads(bc.markets_json) if bc.markets_json else []),
        "conditionsAccepted": bc.conditions_accepted_json if isinstance(bc.conditions_accepted_json, (list, dict)) else (json.loads(bc.conditions_accepted_json) if bc.conditions_accepted_json else []),
        "financingTypes": bc.financing_types_json if isinstance(bc.financing_types_json, (list, dict)) else (json.loads(bc.financing_types_json) if bc.financing_types_json else []),
